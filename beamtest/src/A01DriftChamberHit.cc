...
const std::map<G4String,G4AttDef>* A01DriftChamberHit::GetAttDefs() const
{
  ...
  if (isNew) {
    ...
    G4String ParticleID("ParticleID");
    (*store)[ParticleID] = G4AttDef(ParticleID,"ParticleID","Physics","","G4int");
  }
  return store;
}
...
std::vector<G4AttValue>* A01DriftChamberHit::CreateAttValues() const
{
  ...
  values->push_back
    (G4AttValue("Energy",G4BestUnit(fEnergy,"Energy"),""));

  values->push_back
    (G4AttValue("Pos",G4BestUnit(fWorldPos,"Length"),""));

  values->push_back(G4AttValue("ParticleID",G4UIcommand::ConvertToString(fParticleID),""));
...
{
  G4cout << "  Layer[" << fLayerID << "] : energy " << fEnergy
         << " MeV --- local (x,y) " << fLocalPos.x()
         << ", " << fLocalPos.y()
	 << " --- Particle ID " << fParticleID << G4endl;
}
