...
// ********************************************************************
//
// $Id: GEMEventAction.cc,v 1.10 2007-05-17 09:55:14 duns Exp $
// --------------------------------------------------------------
//
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  verboseLevel = 1;
  messenger = new GEMEventActionMessenger(this);
  ...
  GEMAnalysisManager* analysisManager = GEMAnalysisManager::getInstance();
...
{
#ifdef G4ANALYSIS_USE
  GEMAnalysisManager::dispose();
#endif // G4ANALYSIS_USE
  delete messenger;
}

void GEMEventAction::BeginOfEventAction(const G4Event*)
{
}
...
//    HCHC = (GEMHadCalorimeterHitsCollection*)(HCE->GetHC(HCHCID));
  ...
  if (DHC1 && dc1Hits)
  {
    ...
    {
      GEMDriftChamberHit* aHit = (*DHC1)[i1];
  ...
  if (DHC2 && dc2Hits)
  {
    ...
    for(int i1=0;i1<n_hit;i1++)
    {
      GEMDriftChamberHit* aHit = (*DHC2)[i1];
      G4ThreeVector localPos = aHit->GetLocalPos();
...
/*	if(ECHC)
    {
      ...
      {
        GEMEmCalorimeterHit* aHit = (*ECHC)[i1];
    ...
    if(HCHC)
    {
      ...
      {
        GEMHadCalorimeterHit* aHit = (*HCHC)[i1];
        double eDep = aHit->GetEdep();
        if(eDep>0.)
        {
          iHit++;
